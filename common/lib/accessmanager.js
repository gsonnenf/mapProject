...
export class AccessManager {
    ...
    securePublish(name, accessKey, permissionList, callbacks) {
        var self = this;

        var list = [];

        var accessPub = function() {
        ...
        var documentPub = function() {
            var key = (typeof accessKey == "function") ? accessKey.apply(this) : accessKey;
            return self.getDocCursorFromAccessCursor( self.getAccessCursorByKeyAndPermAny( key, permissionList) );
        };

        Meteor.publish(name + "Access", accessPub);
        Meteor.publish(name + 'Document', documentPub);

        return {
            accessPub: accessPub,
            documentPub: documentPub
        }
    };
};
